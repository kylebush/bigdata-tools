...
sudo cat <<UPSTART >> /etc/init/kafka-manager.conf
...
script
    ...
    exec /srv/kafka-manager/bin/kafka-manager -Dkafka-manager.zkhosts="${ZK_HOSTS}"
...
sudo init-checkconf /etc/init/kafka-manager.conf
